...
//   int temperature_analog_avg = read_temp_avg(temperaturePin, 20);
   int temperature_analog_avg = read_temp(temperaturePin);
...
//   int knob_analog_avg = read_temp_avg(knobPin, 20);
   int knob_analog_avg = read_temp(knobPin);
...
// This is a function takes a single sample of the thermistor
int read_temp(int pin){

    //This function takes as INPUT ARGUMENTS the pin which we want to sample.
    
    int T_avg = analogRead(pin);
